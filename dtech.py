...
# ---------------------------------------------------------------------------
# Markdown export (for Obsidian raw/ ingest)
# ---------------------------------------------------------------------------

_MERMAID_BLOCK_RE = re.compile(r'<pre class="mermaid">(.*?)</pre>', re.DOTALL)


def _html_summary_to_md(html: str) -> str:
    """Convert a stored HTML summary to markdown, preserving mermaid fences.

    ``markdownify`` would otherwise fold ``<pre class="mermaid">`` into a plain
    indented code block and Obsidian would stop rendering the diagram, so we
    stash mermaid bodies with placeholders and splice them back as fenced
    ``` ```mermaid ``` blocks after conversion.
    """
    blocks: list[str] = []
    ...
    staged = _MERMAID_BLOCK_RE.sub(_stash, html)
    body = _md_convert(staged, heading_style="ATX", bullets="-", strip=["span"])
...
def generate_markdown_report() -> Path:
    """Render today's briefing as markdown and mirror it into the Obsidian vault.

    Writes ``daily_report.md`` next to the HTML, copies the same file into
    ``OBSIDIAN_RAW_DIR`` as ``daily-tech-YYYY-MM-DD.md``, and prints an
    ingest reminder so the wiki session knows to file it.
    """
    _init_db()
    ...
    slug = f"daily-tech-{today_iso}"

    lines: list[str] = [
        "---",
        f"title: Daily Tech Report · {today_iso}",
        "type: source",
        f"created: {today_iso}",
        f"updated: {today_iso}",
        "source_format: daily-tech-bot",
        "tags: [daily-brief, tech, papers, releases]",
        "---",
        "",
        f"# Daily Tech Report · {today_human}",
        "",
        (
            f"> Auto-generated by `dtech.py`. "
            f"{len(paper_rows)} paper(s) · {len(github_rows)} repo/release item(s)."
        ),
        "",
    ]

    if paper_rows:
        lines += [f"## Research Papers ({len(paper_rows)})", ""]
        for row in paper_rows:
            label = nice_source_label(row["source"])
            body = _html_summary_to_md(_postprocess_summary(row["summary"]))
            lines += [f"### {label}", "", body, ""]

    if github_rows:
        lines += [f"## GitHub and Releases ({len(github_rows)})", ""]
        for row in github_rows:
            label = nice_source_label(row["source"])
            ts = datetime.fromisoformat(row["timestamp"]).strftime("%b %d · %H:%M")
            body = _html_summary_to_md(_postprocess_summary(row["summary"]))
            lines += [f"### {label}", f"*{ts}*", "", body, ""]
    ...
    if relationship_row:
        lines += [
            ...
            "",
            "```mermaid",
            _sanitize_mermaid(relationship_row[0]).strip(),
            "```",
            "",
        ]
    ...
    if novel_idea_row:
        lines += [
            "### Novel Idea",
            "",
            _html_summary_to_md(_postprocess_summary(novel_idea_row[0])),
            "",
        ]
    ...
    lines += [
        ...
        "",
        (
            f"*Placed in your Obsidian `raw/` folder as `{slug}.md`. "
            f"Open the Second Brain session and say "
            f"`ingest {slug}` to file it into the wiki.*"
        ),
        "",
    ]
    ...
    log.info("Markdown report generated: %s", MARKDOWN_REPORT_PATH)

    vault_path = _mirror_to_obsidian_raw(markdown, slug)
    _remind_to_ingest(slug, vault_path)
    return MARKDOWN_REPORT_PATH


def _mirror_to_obsidian_raw(markdown: str, slug: str) -> Path | None:
    """Write the markdown into the Obsidian vault's raw/ folder.

    Returns the written path on success, None if the vault is unreachable
    (e.g. external drive not mounted) so the pipeline does not crash.
    """
    try:
        OBSIDIAN_RAW_DIR.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        log.warning("Obsidian raw/ unreachable at %s: %s", OBSIDIAN_RAW_DIR, exc)
        return None

    vault_file = OBSIDIAN_RAW_DIR / f"{slug}.md"
    try:
        vault_file.write_text(markdown, encoding="utf-8")
    except OSError as exc:
        log.warning("Could not write markdown to vault: %s", exc)
        return None
    log.info("Copied to Obsidian raw/: %s", vault_file)
    return vault_file


def _remind_to_ingest(slug: str, vault_file: Path | None) -> None:
    """Print a conspicuous reminder so the user remembers to ingest the report."""
    banner = "=" * 66
    print()
    print(banner)
    if vault_file is None:
        print("  Markdown export ready, but vault copy failed.")
        print(f"  Local file: {MARKDOWN_REPORT_PATH}")
        print("  Set OBSIDIAN_RAW_DIR env var or copy it manually to your raw/ folder.")
    else:
        print("  Reminder: ingest today's briefing into your wiki.")
        print(f"  File:    {vault_file}")
        print(f"  Command: ingest {slug}")
        print("  (Open the Second Brain session and paste the command above.)")
    print(banner)
    print()
